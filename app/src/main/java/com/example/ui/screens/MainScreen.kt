...
fun MainScreen(navController: NavController) {
    ...
    val hideThreshold = 120.dp // Ensure we scroll enough to hide completely including insets
    val hideThresholdPx = with(LocalDensity.current) { hideThreshold.roundToPx().toFloat() }
    ...
    val nestedScrollConnection = remember {
        object : NestedScrollConnection {
            override fun onPreScroll(available: Offset, source: NestedScrollSource): Offset {
                ...
                bottomBarOffsetHeightPx.floatValue = newOffset.coerceIn(-hideThresholdPx, 0f)
                return Offset.Zero
            }
        }
    }
